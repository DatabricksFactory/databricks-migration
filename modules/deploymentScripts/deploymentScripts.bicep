...
param ctrlSyntaxType string

param ctrlSourceNotebook string
...
var scriptParametersToUploadFile = '-RG_NAME ${resourceGroup().name} -REGION ${location} -WORKSPACE_NAME ${workspaceName} -LIFETIME_SECONDS ${lifetimeSeconds} -COMMENT ${comment} -CLUSTER_NAME ${clusterName} -SPARK_VERSION ${sparkVersion} -AUTOTERMINATION_MINUTES ${autoTerminationMinutes} -NUM_WORKERS ${numWorkers} -NODE_TYPE_ID ${nodeTypeId} -DRIVER_NODE_TYPE_ID ${driverNodeTypeId} -RETRY_LIMIT ${retryLimit} -RETRY_TIME ${retryTime} -CTRL_DEPLOY_CLUSTER ${(ctrlDeployCluster ? '$true' : '$false')} -MINWORKERS ${minWorkers} -MAXWORKERS ${maxWorkers} -PIPELINENAME ${pipelineName} -STORAGE ${storagePath} -TARGETSCHEMA ${targetSchemaName} -CTRL_DEPLOY_NOTEBOOK ${(ctrlDeployNotebook ? '$true' : '$false')} -CTRL_DEPLOY_PIPELINE ${(ctrlDeployPipeline ? '$true' : '$false')} -NOTEBOOK_PATH ${notebookPath} -SRC_FILESOURCE ${ctrlSourceNotebook == 'RawFileSource' ? '$true' : '$false'} -SRC_AZSQL ${ctrlSourceNotebook == 'AzureSQL' ? '$true' : '$false'} -SRC_AZMYSQL ${ctrlSourceNotebook == 'AzureMySQL' ? '$true' : '$false'} -SRC_AZPSQL ${ctrlSourceNotebook == 'AzurePostgreSQL' ? '$true' : '$false'} -SRC_SQL_ONPREM ${ctrlSourceNotebook == 'SQL_On_Prem' ? '$true' : '$false'} -SRC_PSQL_ONPREM ${ctrlSourceNotebook == 'PostgreSQL_On_Prem' ? '$true' : '$false'} -SRC_ORACLE ${ctrlSourceNotebook == 'Oracle' ? '$true' : '$false'} -SRC_EVENTHUB ${ctrlSourceNotebook == 'Eventhub' ? '$true' : '$false'} -CTRL_SYNTAX ${ctrlSyntaxType} -SA_NAME ${sa_name} -SA_EXISTS ${saExists ? '$true' : '$false'} -SUBSCRIPTION_ID ${subscriptionId} -CTRL_DEPLOY_SAMPLE ${ctrlDeploySample ? '$true' : '$false'}'
