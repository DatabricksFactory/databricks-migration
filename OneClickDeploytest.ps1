...
try {
    Write-Host "Attempt 1 for generating token"
    $DB_PAT = ((Invoke-RestMethod -Method POST -Uri "https://easteeus.azuredatabricks.net/api/2.0/token/create" -Headers $HEADERS -Body $BODY).token_value)
    Write-Output "$DB_PAT"
}
catch {
    Write-Host "Error in Attempt 1 while calling the Databricks API for generating Personal Access Token"
    $errorMessage = $_.Exception.Message
    Write-Host "Error message: $errorMessage"

try {
    Write-Host "Attempt 2 for generating token"
    ...
    Write-Host "Error in Attempt 2 while calling the Databricks API for generating Personal Access Token"
    $errorMessage = $_.Exception.Message
    Write-Host "Error message: $errorMessage"
    }    
}
